...
.get(function(request,response){
    ...
    let arreglo=[];
    basededatos.ref('usuarios').once('value').then(datos => {
...
.post( function(request,response,next){
    ...
    basededatos.ref("CitasMedicasClientes/"+ request.body.cedula).set(request.body).then(p=>{
...
basededatos.ref('usuarios/'+request.params.cedula).once('value').then(dato=>{
    response.send(dato.val());
  })
})
//método DELETE se debe enviar identificación
.delete(function(request,response,next){
    basededatos.ref('CitasMedicasClientes/'+ request.params.cedulax).remove().then(p=>{
