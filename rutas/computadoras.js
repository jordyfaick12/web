...
.get(function(request,response){
    ...
    let arreglo=[];
    basededatos.ref('computadoras').once('value').then(datos => {
...
.post( function(request,response,next){
    ...
    basededatos.ref("computadoras/"+ request.body.identificacion).set(request.body).then(p=>{
...
basededatos.ref('computadoras/'+request.params.identificacionx).once('value').then(dato=>{
    response.send(dato.val());
  })
})
//método DELETE se debe enviar identificación
.delete(function(request,response,next){
    basededatos.ref('computadoras/'+ request.params.identificacionx ).remove().then(p=>{
